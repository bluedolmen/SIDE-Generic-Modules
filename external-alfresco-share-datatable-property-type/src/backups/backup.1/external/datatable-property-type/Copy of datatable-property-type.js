YAHOO.widget.DataTable.prototype.refreshRow = function(row) {
	var self = this;
	Dom.getElementsByClassName(YAHOO.widget.DataTable.CLASS_LINER,'div',this.getTrEl(row),function(el) {
...
var DTP = DTP || {
	...
	*/
	datatableEvents: function(){
		...
		this.computeTotal = function(oArgs) {
			...
			record.setData("totalPrice", formatCurrency(totalPrice));
			var el = this.getTdLinerEl(elRow.cells[4]);
			el.innerHTML = record.getData("totalPrice");
		};

		DTP.dtpDatatable.subscribe("editorSaveEvent", this.computeTotal);
	...
	getTableDefinition: function(){
		...
		for (var e = 0; e < definition.length; e++)
		{
			...
			if (columns && columns.length == 3) definition[e] = [ DTPtrim(columns[0]), DTPtrim(columns[1]), DTPtrim(columns[2]), null ];
				...
				var l = [];
				for (i = 6; i < columns.length; i++) l.push(DTPtrim(columns[i], ["'", "[", "]"]));
				definition[e] = [
				    DTPtrim(columns[0]),
				    DTPtrim(columns[1]),
				    DTPtrim(columns[2]),
...
/*			"key" : c[0], "label" : c[1], "type" : c[2], formatter: getFormatter(c[2]), sortable: true, resizeable: true, editor: getEditor(c[2], c[3]) */
