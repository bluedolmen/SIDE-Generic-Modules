...
SIDE.custom.requisition.Order = {
	...
	computeAllotment: function() {
		...
		function e(x) {
			return SIDE.Util.getElementById("Requisition_ch_unog_dcm_oime_its_Order_" + x);
		}

		/* Get the corresponding float value */
		function f(x) {
			return SIDE.Util.getFloatById("Requisition_ch_unog_dcm_oime_its_Order_" + x);
		}

		if (SIDE.custom.requisition.Order.allotment && SIDE.custom.requisition.Order.expended) {
			e("balance").value   = f("allotment") - f("expended");
			e("committed").value = f("amount");
			e("available").value = f("balance") - f("committed");
		}
	},
	...
	onDataReady: function(type, args, me) {
		...
		// In fact, it is prop_Requisition_ch_unog_dcm_oime_its_Order_amount
		var elt = SIDE.Util.getElementById("Requisition_ch_unog_dcm_oime_its_Order_amount");
